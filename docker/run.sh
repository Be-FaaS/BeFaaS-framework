...
docker run -it --rm --net host --env-file $DOCKER_DIR/env.list $dOpts -e HOST_EXPERIMENT_DIR=$expDir -v $HOME/.azure:/root/.azure -v $expDir:/experiments -v /var/run/docker.sock:/var/run/docker.sock befaas/framework
